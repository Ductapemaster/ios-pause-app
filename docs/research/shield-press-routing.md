...
- `Sources/PauseCore/PauseCountdown.swift:243-246` — `sceneDidLeaveForeground()` is the only place `hasHandledCurrentActivation` is set back to `false`.
- `Sources/PauseCore/PauseCountdown.swift:255-258` — `activate` opens with `guard !hasHandledCurrentActivation else { return .unchanged }`. With the flag still `true` from the earlier foreground, the guard fires and the intent is left unconsumed.
...
That exclusion protects a running countdown from a notification banner or a Control Center pull, and it is right for that. It is wrong at this edge: a shield press arrives through the same `.inactive`-only path when Pause is already frontmost behind the app switcher, and there the press is real intent that gets discarded. The finding is that one flag decides both cases and cannot tell them apart. What to do about it is a product call and is not decided here.
