...
- **A save states its opinion by rebuilding.** Touched is inferred from the resulting document rather than declared by the screen, so a save that changes nothing reads as untouched and carries a scheduled change forward. The rule editor and the settings controls cannot reach it: a pending change locks the controls of the thing it changes, so there is no save to make while one is standing. The app picker still can: re-ticking an app pending removal saves a candidate whose unit for that app matches what is already in force, which the router reads as no opinion, so the removal survives and the re-add does nothing.
