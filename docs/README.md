...
**The app icon is generated from the app's own visual constants.** `Tools/make_app_icon.py` renders the 1024x1024 asset from the indigo of `Color.pauseTint` and the ring ratios and containment rule of `PulsingCircles`, so the home screen and the pause screen stay the same design rather than two drawings that have to be kept in step by hand. Changing the artwork means changing a constant and re-running.

**A waiting shield intent is what makes a return a new activation.** Pause ignores `.inactive` so that a countdown survives a banner or a Control Center pull, and only `.background` marks the next activation as new. Leaving by the app switcher never reaches `.background`, so on its own that rule would discard a shield press made from there and leave the user on whatever screen Pause was showing. The shield action writes its intent before Pause comes forward and a banner writes nothing, so an unconsumed intent marks the return as a new activation. [The investigation](research/shield-press-routing.md).
...
- **The icon is a flat PNG, not a layered one.** iOS composes its own default treatment over it rather than the depth an Icon Composer `.icon` bundle would give. Accepted: the layered format is authored in a GUI, and the flat asset reads correctly at home-screen size.
