...
These are test instruments, not product features. They must be excluded from Release builds. Remove the temporary expiry display and control code after the device evidence is recorded.
...
| Warning callback activity stop | Blocked until the selected-rule stored-`expiresAt` Debug display exists; at `t+00:00` start the first three-minute session, confirm its warning expiry at `t+03:00`, start a second three-minute session at `t+13:30`, observe the first activity's original interval end at `t+15:00`, then observe the second stored expiry at `t+16:30`. | The old interval end at `t+15:00` does not cancel or re-shield the newer grant; the second grant stays open until `t+16:30` and then restores normally without a duplicate charge. | Blocked — required Debug fixture is not implemented; no device observation. | Blocked |
