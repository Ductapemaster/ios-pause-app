...
This entry previously rested on short and long sessions expiring on *different* callbacks. They do not: a 3-minute session was measured on 2026-08-23 delivering both `intervalWillEndWarning` and `intervalDidEnd`, 32 ms apart. `intervalDidEnd` is therefore already exercised on every short session — what remains untested is the long-session case, where no warning accompanies it.
