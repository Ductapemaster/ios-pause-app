...
**Owed on the current build: a shield press from behind the app switcher opens the countdown.** Open a covered app's settings in Pause, switch to that app through the app switcher, and press the shield's primary button. The countdown should come up, not the settings screen. The unit suite pins the rule; only the phone shows that iOS delivers this return as `.inactive` to `.active` with the intent already written ([the investigation](research/shield-press-routing.md)).

**Owed on the current build: which route a failed shield press takes.** A press on 2026-08-24 produced no session from a cold launch, and the archive cannot say whether it landed on the rules list or a repair screen ([the investigation](research/shield-press-routing.md)). The route resolution now logs its route and reason at `.notice`, so the next occurrence settles it: collect the archive and read the `Entry route resolved:` line.
