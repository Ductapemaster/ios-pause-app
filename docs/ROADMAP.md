...
**Pair the app icon with the session line on the pause screen.** The icon sits alone above the session line and reads as stranded; putting them on one row makes a single identity element and closes the last visual complaint. Front end only, `Sources/Pause/PauseView.swift` and `ManualReturnView.swift`.

The app's name is not an option in that row and the reasons are settled: neither half of `Label(applicationToken)` can be resized or centred, and the app cannot read `localizedDisplayName` without an entitlement it does not hold ([the evidence](research/screen-time-platform-evidence.md)). The shield extension does hold the name, but carrying it across to the app would work around the entitlement rather than through it. If [the entitlement](ROADMAP.md) is ever requested for a picker Pause owns, a real name becomes available here as a side effect — worth knowing, not worth requesting for a label.

**Owed on the next build: which route a failed shield press takes.** A press that reached Pause without starting a countdown has been traced to the `.unchanged` path — the app never went `.background`, so the activation was treated as already handled and the intent was discarded ([the investigation](research/shield-press-routing.md)). A second press the same evening also produced no session, from a cold launch that cannot have taken that path, and the archive cannot say whether it landed on the rules list or a repair screen. The route resolution now logs its route and reason at `.notice`, so the next occurrence settles it: collect the archive and read the `Entry route resolved:` line.
