...
**Owed on the current build: two visual judgments on the redesigned pause screen.** The app icon is pinned to a 56pt frame rather than a font size, because `Label(applicationToken)`'s icon half has no documented intrinsic size and a frame behaves predictably either way; whether 56 looks right is only answerable on the screen (`Sources/Pause/AppTokenLabel.swift`). The circles expand over 4s, hold 0.9s and contract over 6s; whether that reads as calm or as sluggish is the same kind of question (`Sources/Pause/PulsingCircles.swift`). Each is one value. Neither blocks anything.
