...
**Next step:** Two device checks gate the merge — the sixteen-minute expiry restoration and the scene-interruption split. `docs/ROADMAP.md` under Deferred has what to look for on each. Once they pass, merge this branch and `feat/session-usage-display` into `design/phased-project-plan`.
...
- [ ] Run the two device checks that gate the merge: a sixteen-minute session expiring and leaving its app blocked, and a notification banner raised over a countdown not abandoning the pause.
...
- [ ] Merge `feat/new-app-configuration-flow` and `feat/session-usage-display` into `design/phased-project-plan`, once the two checks pass.
